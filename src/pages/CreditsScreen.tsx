  ...
  return (
    ...
    >
      ...
      <div className="glass-card mx-3 mt-3 mb-2 px-4 py-2.5 flex items-center gap-3 rounded-2xl flex-shrink-0">
        ...
        <h1 className="font-display text-lg text-primary flex-1 text-center">📝 CRÉDITOS</h1>
        <div className="w-16" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto flex items-center justify-center p-4">
        ...
        >
          ...
          <div>
            <h2 className="font-display text-2xl text-primary mb-1">Universos Atômicos</h2>
            <p className="text-muted-foreground text-sm">Aprenda, jogue e domine a tabela periódica!</p>
          </div>
          ...
          <div className="w-full">
            ...
            <div className="flex flex-col gap-2.5">
              {TEAM.map((member, i) => (
                <motion.div
                  key={member.name}
                  className="glass-card px-4 py-3 flex items-center gap-3 border border-border/30"
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.1 + i * 0.08 }}
                >
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center font-display text-sm text-primary">
          ...
          <p className="text-[0.6rem] text-muted-foreground/60">
            © 2025 Universos Atômicos — Projeto Educacional
          </p>
        </motion.div>
      </div>
    </motion.div>
